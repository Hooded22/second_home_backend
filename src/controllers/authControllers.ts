...
export async function validateRegisterCredentials(req: Request<any, any, IUser>, res: Response, next: NextFunction) {
  const { error } = registerValidation(req.body);
  if (!!error) return res.status(400).send(error?.details[0].message);
  next();
}

export async function checkUserExist(req: Request<any, any, IUser>, res: Response, next: NextFunction) {
